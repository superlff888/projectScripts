...
class Logging:
    ...
    def __new__(cls, filename, level, *args, **kwargs):  # cls 类方法 ；通常用self代表实例方法
        ...
        level = level.upper()  # 改成大写,持续集成时会报错,typeError
        # 创建自己的日志收集器
        my_log = logging.getLogger('webAutoTest')  # 将返回的"my_log"赋值给my_log
        ...
        ft = logging.Formatter(
            '%(asctime)s %(levelname)s %(name)s ["%(pathname)s:%(lineno) d行"] [%(filename)s::%(funcName)s()] - %(message)s 页面操作中……',
            "%Y-%m-%d %H:%M:%S")
        # 设置渠道（控制台和文件）日志的输出格式
        l_c.setFormatter(ft)
        l_f.setFormatter(ft)
