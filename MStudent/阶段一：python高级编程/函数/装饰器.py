...
class Log:
    ...
    def log(text):  # 装饰器函数需要带参数的时候，需要装饰器函数入参处接收参数,本质就是定义函数参数
        def decorator(func):  # 若装饰器有参数，那么一定要在装饰器函数体内接收被装饰的原函数(放在第一层嵌套函数)
            @functools.wraps(func)
            def wrapper(*args, **kw):  # *args, **kwargs用以接收原函数的参数
                # print(f'\n3、装饰器参数为：{text}; 原函数为：{func.__name__}')
                # print(f"4、返回并再调用返回的被装饰原始函数func，即{func()}")  # 被装饰函数一般会在装饰器函数中执行，因为装饰器本身就是为了加强被装饰函数
                print(f"wrapper的 args为：{args}")
                print(f"{text}在内嵌函数wrapper中打印")
                @functools.wraps(func)
                def Called(*args, **kwargs):
                    print("又套一层Called内嵌函数")
                    print(f"Called的args为：{args}")
                    @functools.wraps(func)
                    def inner():
                        print("inner是最里层函数")
                        print(f'\n接收的原函数为：{func.__name__}')
                    return inner
                return Called()  # 此处可以定义called函数的参数，一般三层嵌套就可以实现装饰器了，这层仅理解装饰器时用

            print(f"{text}在内嵌函数decorator打印")
            # print(f"\n2、返回并执行{func.__name__}")
            return wrapper  # (func) # 接收被装饰函数,调用时可以传入被装饰函数，但没必要,因为装饰器的意义在于加强原函数的功能
            # 如果

        print(f"{text}在log函数体内打印")
        print(f"\n执行调用log('execute')后，返回函数{decorator.__name__}")
        return decorator
...
isWrapper = now(1)  # 为啥可以传入参数2？看总结就明白了，wrapper(*args, **kw)可接收任意参数
...
1、核心：[装饰器特性]被装饰函数可被自动传给装饰器函数“入参位置”或函数体; 被装饰函数被认为是装饰器函数的实参。
   注意：(1) [函数的参数化处理]被装饰函数是作为参数，既可以放到装饰器函数入参处，也可以放到装饰器函数体中;
...
3、本质就是函数的参数处理+被装饰函数以参数形式自动传递给装饰器函数。
