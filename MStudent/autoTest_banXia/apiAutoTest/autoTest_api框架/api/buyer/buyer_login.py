...
class BuyerLoginApi(BaseBuyerApi):
    ...
    def __init__(self):  # path, method, params
        # 通过ini配置文件处理本接口的入参
        self.USERNAME = conf_parser_obj.configParser(["buyer_login", "username"])
        self.PASSWORD = conf_parser_obj.configParser(["buyer_login", "password"])
        self.PATH = conf_parser_obj.configParser(["buyer_login", "path_login"])
        self.CAPTCHA = conf_parser_obj.configParser(["buyer_login", "captcha"])
        self.METHOD = conf_parser_obj.configParser(["buyer_login", "method_login"])
        self.UUID = conf_parser_obj.configParser(["buyer_login", "uuid"])
