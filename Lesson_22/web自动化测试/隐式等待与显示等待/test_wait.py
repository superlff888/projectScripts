...
class TestWait:
    ...
    def test_wait(self):
        ...
        WebDriverWait(self.driver, 5).until(expected_conditions.visibility_of_element_located((By.CSS_SELECTOR, "#ember31")))

        # 自定义一个方法
        def wait(x):  # 方法中一定要带一个参数，因为until中会将self._driver传给该方法
            return len(self.driver.find_elements(By.XPATH, "//*[class='ember-view']")) >= 1  # 判断元素的个数（长度）
        """
