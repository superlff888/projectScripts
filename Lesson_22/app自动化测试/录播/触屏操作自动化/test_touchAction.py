...
场景一【 需要登录或者特别的状态，就不适合每个都重启，启动一次就好】
    ...
    注意：和 desired_caps['noReset'] = 'true'，只是不清空app缓存情况下启动app；不能控制app启动次数
...
class TestDw:
    ...
    def test_search_2(self):
        ...
        # 方式1：WebDriverWait(self.driver, 10).until(ES.element_to_be_clickable(locator))
        # 方式2：WebDriverWait类init方法中将self.driver转变为成员变量self._driver,until中的method调用成员变量self._driver
        WebDriverWait(self.driver, 10).until(lambda x: x.find_element(*locator))  # 方法中一定要带一个参数，因为until中会将self._driver传给该方法
    ...
    # 待优化
    def teardown(self):
        if not self.driver.find_element_by_id("com.xueqiu.android:id/tv_search").is_displayed():
            self.driver.back()
